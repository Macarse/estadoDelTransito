...
public class AvenuesServlet extends HttpServlet{
	...
	private void respondEs(HttpServletResponse resp) throws IOException {
		...
		String query = "select from " + Access.class.getName() + " where type=='Avenue'" + " order by name";
	...
	private void respondEn(HttpServletResponse resp) throws IOException {
		...
		PersistenceManager pm = PMF.get().getPersistenceManager();
		String query = "select from " + AccessEn.class.getName() + " where type=='Avenue'" + " order by name";
